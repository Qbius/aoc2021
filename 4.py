...
def get_board_score(base, find_f):
    ...
    last, (board, selected) = find_f(drawns, boards)
    return last * sum(e for i, e in enumerate(board) if i not in selected)

def first(base: raw_input):
    return get_board_score(base, find_winning)

def second(base: raw_input):
    return get_board_score(base, find_last_winning)